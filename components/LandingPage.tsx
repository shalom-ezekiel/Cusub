...
const LandingPage: React.FC<LandingPageProps> = ({ onGetStarted }) => {
  ...
  return (
    ...
    <div className="bg-zinc-950 text-zinc-100 min-h-screen overflow-x-clip selection:bg-emerald-500/30 scroll-smooth">
      {/* Floating Pill Navigation */}
      <motion.div
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        transition={{ duration: 0.6, ease: "easeOut" }}
        className="fixed top-4 left-0 right-0 z-[100] px-3 sm:px-4"
      >
                <div className="max-w-2xl mx-auto">
          <nav className="bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/50 rounded-full px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 flex items-center justify-between shadow-2xl shadow-black/80">
            {/* Logo */}
            <div className="flex items-center gap-2 cursor-pointer shrink-0" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>

              <span className="text-lg font-black hidden sm:block">Cusub</span>
            </div>

            {/* Desktop Navigation Links - Hidden on mobile */}
            <div className="hidden lg:flex items-center justify-center gap-6 text-[10px] font-black uppercase text-zinc-500">
         ...
         <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 shrink-0">
              ...
              <button
                onClick={onGetStarted}
                className="px-3 sm:px-4 py-1.5 sm:py-2 bg-emerald-500 text-zinc-950 rounded-full font-black text-[9px] sm:text-[10px] uppercase hover:bg-emerald-400 whitespace-nowrap shrink-0 shadow-lg shadow-emerald-500/20"
              >
                Get Started
              </button>
            ...
            {/* Mobile Menu Button  */}
              <button
                ...
                className="lg:hidden p-1.5 sm:p-2 hover:bg-zinc-800 rounded-full transition-colors shrink-0"
          ...
          <AnimatePresence>
            {mobileMenuOpen && (
              <motion.div
                ...
                className="lg:hidden mt-3 bg-zinc-900/95 backdrop-blur-2xl border border-zinc-800/50 rounded-2xl overflow-hidden shadow-2xl shadow-black/60"
              >
                <div className="flex flex-col">
                  <a
                    href="#problem"
                    onClick={() => setMobileMenuOpen(false)}
                    className="flex items-center gap-3 px-5 py-4 text-xs font-black uppercase tracking-widest text-zinc-400 hover:text-emerald-400 hover:bg-zinc-800/40 transition-all border-b border-zinc-800/30"
                  >
                  ...
                  <a
                    href="#status"
                    onClick={() => setMobileMenuOpen(false)}
                      className="flex items-center gap-3 px-5 py-4 text-xs font-black uppercase tracking-widest text-zinc-400 hover:text-emerald-400 hover:bg-zinc-800/40 transition-all"
                  >
